...
ggplot(nepal_anthro_clean, aes(x = age, y = wt)) +
 ...
 # geom_abline(slope = coef(slr)[["age"]], intercept = coef(slr)[["(Intercept)"]], color = "red") +
  theme_bw() +
  geom_jitter(size = 1, data = nepal_anthro_spline, aes(x = age, y = linear_spline_reg$fitted.values), color = "red")

#linear_spline_reg$fitted.values
```
...
The evidence from this analysis suggests that a linear growth curve is not appropriate for describing the relationship between weight and age in Nepali children. The coefficients for age_sp6 and age_sp12 are not statistically significant, and the confidence intervals for these coefficients include zero. This suggests that the rate of change in weight is not constant across the age intervals 0-6, 6-12, and 12-60 months.
