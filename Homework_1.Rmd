...
is approximately 7 (1) kg, respectively. Average and SD of weight
increases with age such that the average (SD) weight is approximately
11.5 (1.5) and 13 (2) for children aged 36 and 48 months, respectively.
...
In at most three sentences, describe how the average weight differs with
age for female children (use at least 2 reference ages) and if there are
differences comparing males to females. Focus on the average weights and
be quantitative.

The weights of 185 children ranging from 1 to 60 months of age were
plotted as a function of age with separate smoothing lines for males and
females. The average weight of 12 month old females is approximately 7
kg and increases with age so that the average weight of 24 month old
females is approximately 9.5 kg. The average weight for males also
increases with age at a similar rate to that of the females but after
approximately 45 months, the male average weight increases at a higher
rate than that of females.
...
difference in average weight comparing children who in age by one month
is estimated to be 0.157 kg with a 95% confidence interval of (0.145,
0.169) kg. The measure of spread in weight for children of all ages in
this data is 1.4 kg.

In a few sentences, as if for a public health audience, interpret the:
intercept, slope, and residual standard deviation in anthropometric
terms. Include the estimates and confidence intervals in your sentences
to be quantitative but use no statistical jargon (e.g. “intercept”,
“slope”). For example, use “difference in average weight comparing
children who differ in age by one year” rather than “slope”.
...
ggplot(nepal_anthro_clean, aes(x = age, y = wt)) +
  ...
  ylab("Weight at Baseline (kg)") +
  theme_bw()
# add a legend for the red and grey line
...
the figure because the data points in the scatterplot follow the
increasing linear regression line.
...
describes the relationship between weight and age at 3 different age
...
children who differ by one month in age (x months compared to x-1
months) is 0.5285 kg, which is the coefficient for age_c. The 95%
confidence interval for this coefficient is (0.197, 0.86) kg. Among
children 6-12 months of age, the difference in average weight comparing
children who differ by one month in age (comparing children of x months
vs. x-1 months) is 0.186 kg, so the coefficient for age_sp6, -0.3423, is
the difference between the average monthly change in kilograms of weight
...
age_sp12, -0.147, is the difference between the average monthly change
...
from 0-6 months and a flatter slop from 6-12 months and 12-60 months.
...
potential decrease in rate of weight growth for the Nepali children at
...
ggplot(data = hat_plot, aes(x = age)) +
  ...
  labs(title = "Hat Matrix vs. Age for 3 Children at 12, 24, and 36 Months of Age",
       x = "Age at Baseline (months)",
       y = "Hat Matrix Values") +
  theme_bw() + 
  scale_color_manual(breaks = c("1", "2", "3"),
...
Comment on patterns you observe; i.e. what values of Y are most
informative (have the largest weights) for each child’s predicted value?
Is this what you expect based on the mean model you specified? Why or
why not?
...
weights) because the points are concentrated around the child's age. For
...
# compute prediction errors (sum of sqared errors)
...
Conduct a multiple linear regression of weight on age and height. In a
few sentences, as if for a public health audience, interpret the
intercept, age coefficient, and residual standard deviation in
anthropometric terms. Include the estimates and confidence intervals in
your sentences to be quantitative but use no statistical jargon (e.g.
“intercept”, “slope”).
...
variation in weight for children of all ages in this data is 0.916 kg.
...
coefficient may not be very significant in the mnultiple linear
regression model because it is small that depending on variability, it
