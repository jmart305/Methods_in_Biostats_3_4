...
children of any given age in this data is estimated to be 1.4 kg.
...
the figure because the data points in the scatterplot are distributed
...
children 6 to 12 vs. under 6 months of age. A 95% confidence interval
...
kilograms of weight comparing children 12 to 60 vs. 6 to 12 months of
...
ggplot(nepal_anthro_spline2, aes(x = age, y = wt)) +
  ...
  geom_point(aes(x = age, y = cubic_spline_reg$fitted.values), color = "blue", size = 1) +
  scale_color_manual(breaks = c("1", "2"),
                       values = c("red", "blue"),
...
ggplot(nepal_anthro_spline2, aes(x = age, y = wt)) +
  ...
  geom_point(aes(x = age, y = natural_spline_reg$fitted.values), color = "green4", size = 1) +
...
model_pe_noncv <- 
  data.frame(df = seq(1,8,1),
             non_cv_prediction_error = c(model1_pe, model2_pe, model3_pe, model4_pe, model5_pe, model6_pe, model7_pe, model8_pe))

kable(model_pe_noncv, "html", align = "lc",
      col.names = c("Degrees of freedom", "Non-Cross validated prediction error"))
```
...
ggplot(model_pe, aes(x = df, y = cv_prediction_error)) +
  ...
  geom_point(data = model_pe_noncv, aes(x = df, y = non_cv_prediction_error, color = "2")) +
  geom_line(data = model_pe_noncv, aes(x = df, y = non_cv_prediction_error, color = "2")) +
  labs(title = str_wrap("Cross-Validated and Non-Cross-Validated Prediction Error vs. Degrees of Freedom", width = 65),
       x = "Degrees of Freedom",
...
8, I conducted cross-validation to obtain the predicted weight for each
...
entire dataset to fit the model a one time and then make predictions on
...
variation in weight for children of any given age in this data is
estimated to be 0.916 kg.
