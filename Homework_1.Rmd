...
ggplot(nepal_anthro_clean, aes(x = age, y = wt)) +
  ...
  labs(color='Sex', title = "Weight vs. Age of Nepali Children with Natural Spline Smoother (3 df)") + 
...
11.5 (1.4) and 13 (1.8) for children aged 36 and 48 months, respectively.
...
I plotted the weights of 185 children as a function of age with separate smoothing lines for males and females. The average weight of 12 month old females is approximately 7
kg and increases with age so that the average weights of 24 month old and 36 month old
females is approximately 9 kg and 11.5 kg, respectively. The average weight for males is similar to that of females, but after approximately 45 months, the male average weight increases at a higher rate than that of females, so that by 60 months the average weight for males is 14.5 kg and for females is only 13 kg.  
...
difference in average weight comparing children who differ in age by one month
is estimated to be 0.157 kg with a 95% confidence interval of (0.145,
0.169) kg. The measure of spread or variation in weight for children of any given age in
this data is estimated to be 1.4 kg.
...
ggplot(nepal_anthro_clean, aes(x = age, y = wt)) +
  geom_jitter(size = 1, aes(color = as.factor(sex))) +
  geom_smooth(method = "glm", formula = y ~ ns(x, df = 3), se = FALSE, aes(color = "3")) +
  geom_abline(aes(slope = coef(slr)[["age"]], intercept = coef(slr)[["(Intercept)"]], color = "4")) +
  scale_color_manual(breaks = c(1, 2, 3, 4), values = c("orange", "blue", "darkgrey", "red"), labels = c("Male", "Female", str_wrap("Natural Spline Smoother", width = 15), str_wrap("Simple Linear Regression Line", 15))) +
...
the figure because the data points in the scatterplot are distributed closely around the increasing linear regression line. 
...
spread is larger, indicating that constant variation may not be a valid assumption.
...
ggplot(nepal_anthro_spline, aes(x = age, y = wt)) +
  ...
  labs(title = str_wrap("Weight vs. Age of Nepali Children and Fitted Values from Linear Spline Regression Line", width = 70)) + 
  xlab("Age at Baseline (months)") + 
  ylab("Weight at Baseline (kg)") +
  geom_line(aes(x = age, y = linear_spline_reg$fitted.values),color="red") +
  geom_point(aes(x = age, y = linear_spline_reg$fitted.values), color = "red", size = 1) +
  theme_bw()
```
...
describes the linear relationship between weight and age at 3 different age
intervals: 0-6 months, 6-12 months, and 12-60 months.
...
children who differ by one month in age is 0.5285 kg, which is the coefficient for age_c. The 95% confidence interval for this coefficient is (0.197, 0.86) kg. Among
children 6-12 months of age, the difference in average weight comparing
children who differ by one month in age is 0.186 kg, so the coefficient for age_sp6, -0.3423, is the difference between the average monthly change in kilograms of weight
comparing children 6 to 12 vs. under 6 months of age. A 95% confidence
interval for this coefficient is (-0.789, 0.105) kg. Among children
12-60 months of age, the difference in average weight comparing children
who differ by one month in age is 0.147 kg, so the coefficient for
age_sp12, -0.039, is the difference between the average monthly change
in kilograms of weight comparing children 12 to 60 vs. 6 to 12 months of
...
The evidence from this analysis suggests that a linear growth curve may not be the best model for this data because for the interval from 12-60 months, there is a lot more variation in the outcome variable weight (compared to the younger ages) that this linear growth curve does not seem to capture very well. Describing the relationship between age and weight in this data may benefit from a more flexible type of model.  
...
ggplot(nepal_anthro_spline2, aes(x = age, y = wt)) +
  ...
  labs(color='Sex', title = str_wrap("Weight vs. Age of Nepali Children with Fitted Values from Linear Spline and Cubic Spline Regression Lines", width = 70)) + 
  xlab("Age at Baseline (months)") + 
  ylab("Weight at Baseline (kg)") +
...
ggplot(nepal_anthro_spline2, aes(x = age, y = wt)) +
  geom_jitter(size = 1, color = "black") +
  labs(color='Sex', title = str_wrap("Weight vs. Age of Nepali Children with Fitted Values from Linear, Cubic, and Natural Splines", width = 70)) + 
  xlab("Age at Baseline (months)") + 
  ylab("Weight at Baseline (kg)") +
  geom_line(aes(x = age, y = linear_spline_reg$fitted.values,color="1")) + 
  geom_line(aes(x = age, y = cubic_spline_reg$fitted.values,color="2")) + 
...
hat_plot <- data.frame(child_12 = child_12_hat, child_24 = child_24_hat, child_36 = child_36_hat, age = nepal_anthro_clean$age)
...
weights) because the points are concentrated around each child's age. For
example, for the 24 month old child, there is a peak around 18-24
...
The natural spline model with 2 degrees of freedom results in the lowest cross-validation prediction error and therefore the best predictions of the data. 
...
model1_pe <- sum((pred_weight$wt - pred_1)^2)
model2_pe <- sum((pred_weight$wt - pred_2)^2)
model3_pe <- sum((pred_weight$wt - pred_3)^2)
model4_pe <- sum((pred_weight$wt - pred_4)^2)
model5_pe <- sum((pred_weight$wt - pred_5)^2)
model6_pe <- sum((pred_weight$wt - pred_6)^2)
model7_pe <- sum((pred_weight$wt - pred_7)^2)
model8_pe <- sum((pred_weight$wt - pred_8)^2)
```

```{r}
model_1_noncv <- lm(wt ~ ns(age, 1), data = train_data)

p <- predict(model1_train, newdata = train_data)
sum((train_data$wt - p)^2)
```
...
ggplot(nepal_anthro_clean, aes(x = age, y = wt)) +
  geom_jitter(size = 1, color = "blue") +
  labs(title = str_wrap("Weight vs. Age of Nepali Children and Fitted Values from Natural Spline with 2 df", width = 70)) + 
  xlab("Age at Baseline (months)") + 
  ylab("Weight at Baseline (kg)") +
  geom_line(aes(x = age, y = model_2_noncv$fitted.values),color="red") +
  theme_bw()
```
...
I used cross-validation to select the optimal natural spline model to describe the relationship between weight and age of 185 Nepali children. For 8 different natural spline models with degrees of freedom from 1 to 8, I conducted cross-validation to obtain the predicted weight for each child. I split the data into 10 folds and for each of the 10 iterations, one of the folds was used as the test set while the other 9 folds were used to train the model. In this way, the predictions were obtained using data that was not used to fit the model. To calculate the prediction errors for each of the 8 models, I summed the squared differences between the predicted and observed weights for each child and displayed these results in a table and plot. 
**METHODS FOR NON-CV**
I then calculated prediction errors using a non-cross-validation procedure/approach and 
displayed in a table. 

The cross-validated prediction error was lowest for the natural spline model with 2 degrees of freedom and had a value of 341.88. The highest cross-validated prediction errors were 366.87 and 365.83, which correspond to the natural spline models with 1 and 8 degrees of freedom, respectively. This suggests that the natural spline model with 2 degrees of freedom is the best model for predicting weight based on age. ***RESULTS FOR NON-CV*** 
...
variation in weight for children of any given age in this data is estimated to be 0.916 kg.
...
```{r}
compare_age_coef <- data.frame(age = coef(mlr)[2], adjusted_age = coef(adjust_model)[2])
compare_age_coef
```
...
regression model because it is so small that, depending on variability, it
